...
typerCssArray.push(args => {
  ...
  let currentWordNotWrong = inputWords.length <= verseWords.length && getTransformedWord(args, verseWords[Math.max(inputWords.length - 1, 0)] ?? "").indexOf(getTransformedWord(args, inputWords[inputWords.length - 1] ?? "")) === 0;
